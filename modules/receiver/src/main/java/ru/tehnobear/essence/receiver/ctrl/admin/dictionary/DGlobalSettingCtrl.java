...
import ru.tehnobear.essence.receiver.service.admin.dictionary.DGlobalSettingService;
...
@Slf4j
@Tag(name = "Dictionary Admin DGlobalSetting", description = "Справочник глобальных переменных")
public class DGlobalSettingCtrl implements DictionaryApi {
    private final DGlobalSettingService service;
    private final UserService userService;
    private final AuditService auditService;
    @Operation(summary = "Fetch DGlobalSetting")
    @PostMapping(value = "/DGlobalSetting", produces = {MediaType.APPLICATION_JSON_VALUE})
    ...
    public Mono<DGlobalSettingResult> fetchDGlobalSetting(@RequestBody DGlobalSettingFetch fetch) {
        return service.fetch(fetch);
    }
    @Operation(summary = "Insert DGlobalSetting")
    @PutMapping(value = "/DGlobalSetting", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> insert(
            @RequestBody DGlobalSettingInsert data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Update DGlobalSetting")
    @PatchMapping(value = "/DGlobalSetting", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> update(
            @RequestBody DGlobalSettingUpdate data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Delete DGlobalSetting")
    @DeleteMapping(value = "/DGlobalSetting", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> delete(@RequestBody DGlobalSettingDelete data) {
        return userService
                .getUser()
                .flatMap(user -> {
                    var tLog = TLog.builder()
