...
import ru.tehnobear.essence.receiver.service.admin.dictionary.DSourceTypeService;
...
@Slf4j
@Tag(name = "Dictionary Admin DSourceType", description = "Справочник типов источников данных")
public class DSourceTypeCtrl implements DictionaryApi {
    private final DSourceTypeService service;
    private final UserService userService;
    private final AuditService auditService;
    @Operation(summary = "Fetch DSourceType")
    @PostMapping(value = "/DSourceType", produces = {MediaType.APPLICATION_JSON_VALUE})
    ...
    public Mono<DSourceTypeResult> fetchDSourceType(@RequestBody DSourceTypeFetch fetch) {
        return service.fetch(fetch);
    }
    @Operation(summary = "Insert DSourceType")
    @PutMapping(value = "/DSourceType", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> insert(
            @RequestBody DSourceTypeInsert data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Update DSourceType")
    @PatchMapping(value = "/DSourceType", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> update(
            @RequestBody DSourceTypeUpdate data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Delete DSourceType")
    @DeleteMapping(value = "/DSourceType", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> delete(@RequestBody DSourceTypeDelete data) {
        return userService
                .getUser()
                .flatMap(user -> {
                    var tLog = TLog.builder()
