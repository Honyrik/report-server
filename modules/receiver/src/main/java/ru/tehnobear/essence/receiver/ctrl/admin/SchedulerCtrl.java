...
import ru.tehnobear.essence.receiver.service.admin.SchedulerService;
...
@Slf4j
@Tag(name = "Admin Scheduler", description = "Управление Scheduler")
public class SchedulerCtrl implements AdminApi {
    private final SchedulerService service;
    private final UserService userService;
    private final AuditService auditService;
    @Operation(summary = "Fetch Scheduler")
    ...
    public Mono<SchedulerResult> fetchScheduler(@RequestBody SchedulerFetch fetch) {
        return service.fetch(fetch);
    }
    @Operation(summary = "Insert Scheduler")
    @PutMapping(value = "/Scheduler", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> insert(
            @RequestBody SchedulerInsert data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Update Scheduler")
    @PatchMapping(value = "/Scheduler", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> update(
            @RequestBody SchedulerUpdate data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Delete Scheduler")
    @DeleteMapping(value = "/Scheduler", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> delete(@RequestBody SchedulerDelete data) {
        return userService
                .getUser()
                .flatMap(user -> {
                    var tLog = TLog.builder()
