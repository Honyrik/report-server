...
import ru.tehnobear.essence.receiver.service.admin.ReportAssetService;
...
@Slf4j
@Tag(name = "Admin ReportAsset", description = "Управление ReportAsset")
public class ReportAssetCtrl implements AdminApi {
    private final ReportAssetService service;
    private final UserService userService;
    private final AuditService auditService;
    @Operation(summary = "Fetch ReportAsset")
    @PostMapping(value = "/ReportAsset", produces = {MediaType.APPLICATION_JSON_VALUE})
    ...
    public Mono<ReportAssetResult> fetchReportAsset(@RequestBody ReportAssetFetch fetch) {
        return service.fetch(fetch);
    }
    @Operation(summary = "Insert ReportAsset")
    @PutMapping(value = "/ReportAsset", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> insert(
            @RequestBody ReportAssetInsert data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Update ReportAsset")
    @PatchMapping(value = "/ReportAsset", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> update(
            @RequestBody ReportAssetUpdate data
    ) {
        return userService
                .getUser()
                .flatMap(user -> {
    ...
    @Operation(summary = "Delete ReportAsset")
    @DeleteMapping(value = "/ReportAsset", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiResponse(responseCode = "200", description = "OK")
    @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = Void.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = Void.class)))
    @PreAuthorize(Roles.ROLE_ADMIN_AUTHORITY)
    public Mono<Result> delete(@RequestBody ReportAssetDelete data) {
        return userService
                .getUser()
                .flatMap(user -> {
                    var tLog = TLog.builder()
