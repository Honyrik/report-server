...
@Slf4j
public class JRJSONExporter implements JasperFormatExporter {
    public final String name;
    private final Map<String, Object> param;
    private TDFormat data;
    public JRJSONExporter(TDFormat format, Map<String, Object> param) {
    ...
    public byte[] export(JasperPrint jasperPrint, TQueue queue) {
        try {
            ...
            exporter.setExporterOutput(new SimpleJsonExporterOutput(byteArrayOutputStream));
            var property = new HashMap<>(param);
            property.putAll(queue.getCctParameter());
            var configuration = Util.objectMapper.convertValue(property, SimpleJsonExporterConfiguration.class);
